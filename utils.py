...
def set_sumo(gui, sumocfg_file_name, max_steps):
    ...
    """
    # import modules from the $SUMO_HOME/tools directory to Python path
    ...
    # -c: config, --no-step-log (true): turn off logging for each simulation step, --waiting-time-memory (max_steps): Memory limit for storing vehicle waiting time information.
    return sumo_cmd
...
def set_train_path(models_path_name):
    ...
    if dir_content:
        previous_versions = [int(name.split("_")[1]) for name in dir_content] # E.g.: model_2.split("_")[1] = 2
