...
class TrafficGenerator:
    def __init__(self, max_steps, n_cars_generated):
        ...
        self._max_steps = max_steps # max simulation steps per episode
    ...
    def generate_routefile(self, seed):
        ...
        for value in timings:
            ...
            # Linearly rescale each generated timing value to fit the new range (0 to max_steps)
        car_gen_steps = np.rint(car_gen_steps)  # round every value to int -> effective steps when a car will be generated

        # produce the file for cars generation, one car per line
        with open("intersection/episode_routes.rou.xml", "w") as routes:
            # acceleration, deceleration rate, length of cars, min gap between 2 vehicles.
            # Sigma is randomness. Lower sigma value - the vehicle has more stable behavior and vice versa.
            print("""<routes>
            ...
            # Edges are South to (2) traffic light and traffic light to East
            ...
            for car_counter, step in enumerate(car_gen_steps):
                ...
                if straight_or_turn < 0.75:  # choose direction: straight or turn - 75% of times the car goes straight
                    ...
                    if route_straight == 1:
                        print('    <vehicle id="W_E_%i" type="standard_car" route="W_E" depart="%s" departLane="random" departSpeed="10" />' % (car_counter, step), file=routes) # Depart in a random lane on the road
